...
/**
 * The Chessboard class represents the internal chessboard of SloppyGUI.
 *
...
class Chessboard
{
	public:
		...
		enum ChessSide
		{
			...
			Black,
			/** No side. Used when an engine is in force (observer) mode. */
			NoSide
		};
		...
		enum ChessSquare
		{
			...
			NoSquare /**< Any square that is not on the board. */
		};
		...
		enum ChessPiece
		{
			/**
			 * All chess pieces.
			 * Only used as an array index in the Chessboard class.
			 */
			AllPieces,
			Pawn,
			Knight,
			 ...
			 */
			RQ,
			NoPiece /**< No piece. Anything which is not a chess piece. */
		};
		 ...
		 * Doesn't set the board to any specific position.
		...
		/**
		 * Creates a new Chessboard object.
		 * Sets the board to a position defined in a FEN string.
		...
		/**
		 * Makes a move on the board.
		 * @param moveString a move in either SAN or Coordinate notation.
		 * @see undoMove()
		 */
		...
		/**
		 * Initializes the Chessboard class.
